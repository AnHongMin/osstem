...
String.prototype.biznum = function() {
	...
	return (parseInt(num.substring(9, 10)) == 10 - (cVal % 10) % 10) ? true
			: false;
};
...
FormUtil.prototype.success = function() {
	...
	for ( var i = 0; i < this.obj.elements.length; i++) {
		...
		var fname = (f.getAttribute("FNAME") == null) ? f.name.toUpperCase()
				: fname = f.getAttribute("FNAME");
		...
		// radio
			...
			if (f.getAttribute("DENY") != null && f.type == "file"
					&& f.value != "") {
				var ext = f.value.getExt().toLowerCase();
				var ext2 = f.getAttribute("DENY").toLowerCase();
				if (ext2.indexOf(ext) >= 0) {
					alert("확장자가 " + f.getAttribute("DENY").toUpperCase()
							+ " 인 파일은 업로드 하실 수 없습니다.");
					return false;
				}
			}
			// <input type="file" deny="value">
			if (f.getAttribute("ALLOW") != null && f.type == "file"
					&& f.value != "") {
				var ext = f.value.getExt().toLowerCase();
				var ext2 = f.getAttribute("ALLOW").toLowerCase();
				if (ext2.indexOf(ext) < 0) {
					alert("확장자가 " + f.getAttribute("ALLOW").toUpperCase()
							+ " 인 파일만 업로드 가능 합니다.");
					return false;
				}
			}
			...
			if (f.getAttribute("MAX") != null) {
				...
				if (tmpLen > parseInt(f.getAttribute("MAX"))) {
					alert(fname + fname.josa("은/는") + " "
							+ f.getAttribute("MAX") + "자 이하로 입력 하세요.");
					f.value = "";
					f.focus();
					return false;
				}
			...
			if (f.getAttribute("MIN") != null) {
				...
				if (tmpLen < parseInt(f.getAttribute("MIN"))) {
					alert(fname + fname.josa("은/는") + " "
							+ f.getAttribute("MIN") + "자 이상으로 입력 하세요.");
					f.focus();
					return false;
				}
			}
			...
			if (f.getAttribute("LEN") != null) {
				...
				if (val.indexOf(val.noNum()) > 0) {
					...
					var num2 = val.substring(val.lastIndexOf(val.noNum()) + 1,
							val.length);
					if (tmpLen < parseInt(num1) || tmpLen > parseInt(num2)) {
						alert(fname + fname.josa("은/는") + " " + num1 + "자 이상 "
								+ num2 + "자 이하로 입력하세요");
						f.focus();
						return false;
					}
...
FormUtil.prototype.checkbox = function(f, fname) {
	...
	if (f.getAttribute("MAX") != null) {
		...
		if (c > parseInt(val)) {
			alert(fname + fname.josa("은/는") + " 최대 " + val
					+ "개 이하로 선택 하셔야 합니다.");
			return false;
		}
	}
	...
	if (f.getAttribute("LEN") != null) {
		...
		if (val.indexOf(val.noNum()) > 0) {
			...
			if (c < parseInt(num1) || c > parseInt(num2)) {
				alert(fname + fname.josa("은/는") + " " + num1 + "개 이상 " + num2
						+ "개 이하로 선택 하셔야 합니다.");
				return false;
			}
		} else {
...
FormUtil.prototype.getValue = function(name, value) {
	...
	for ( var j = 0; j < this.obj.elements.length; j++) {
		if (eval("this.obj[j].getAttribute(\"" + name + "\")") != null
				&& eval("this.obj[j].getAttribute(\"" + name + "\")") == value) {
			if (val == "") {
