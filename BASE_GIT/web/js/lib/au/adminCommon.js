...
ayu.data.prototype.hasKey = function( key ) {
  if( ayu.chk.isNull(key) ) return false;
...
ayu.ajax.onError = function( xhr ) {
  ...
  errMsg += "----------------------------------------------" + "\n";
  switch( xhr.status ) {
    case 403 : errMsg += "요청하신 주소에 접근할 수 없습니다.\n" ; break;
    case 404 : errMsg += "요청하신 주소를 찾지 못하였습니다.\n" ; break;
	case 500 : errMsg += "위의 오류가 반복적으로 발생하는 경우 화면을 새로고침(F5) 하시기 바랍니다. \n새로고침(F5) 시 작성중인 내용은 유실될 수 있습니다." ; break;
    case 0   : errMsg += "서버와 연결하지 못하였습니다.\n" ; break;
  }
  errMsg += "----------------------------------------------" + "\n";
...
ayu.window.addData = function( data ) {
  if( (data._id != ayu.c.MAP.ID_DATA ) || data.size() == 0 ) return;
...
ayu.window.getForm = function( data ) {
  ...
  if( form.length == 0 ) {
    ...
    $("<form name='xecure'><input type=hidden name='p'/></form>").appendTo( "body" );
    
    form = $("<form>")
      .attr("id", ayu.c.WINDOW.FORM)
  ...
  // 데이터가 없다면 메서드를 종료한다.
  if( ayu.chk.isNull(data) ) return form;
